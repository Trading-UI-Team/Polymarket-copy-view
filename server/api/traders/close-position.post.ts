export default defineEventHandler(async (event) => {
    ...
    if (!asset || typeof asset !== 'string') {
        throw createError({ statusCode: 400, message: 'Missing or invalid asset' })
    }
        ...
        const confirmation: any = await new Promise(async (resolve, reject) => {
            ...
            const timer = setTimeout(async () => {
                if (!isResolved) {
                    ...
                    reject(createError({ statusCode: 504, message: 'Timeout waiting for position close confirmation' }))
                }
            }, timeoutMs)

            try {
                subscription = await subscribeNotifications(async (msg: any) => {
                    if (msg.event === 'position_closed' && msg.taskId === taskId && msg.asset === asset) {
                        if (!isResolved) {
                            isResolved = true
                            clearTimeout(timer)
                            if (subscription) await subscription.cleanup()
